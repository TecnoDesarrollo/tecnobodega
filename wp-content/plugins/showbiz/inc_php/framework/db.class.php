<?php
	...
	class UniteDBBiz{
		...
		private function throwError($message,$code=-1){
			UniteFunctionsBiz::throwError($message,$code);
		}
		...
		public function delete($table,$where){
			...
			UniteFunctionsBiz::validateNotEmpty($table,"table name");
			UniteFunctionsBiz::validateNotEmpty($where,"where");
		...
		public function update($table,$arrItems,$where){
			...
			if($response === false)
				UniteFunctionsBiz::throwError("no update action taken!");
		...
		/**
		 * 
		 * show native table content
		 */		
		public function showTableContent($table,$usePrefix=true){
			global $wpdb;
			
			if($usePrefix == true)
				$table = $wpdb->prefix.$table;
				
			$arr = $this->fetch($table);
			dmp($arr);
