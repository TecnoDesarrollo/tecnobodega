	...
	class UniteFunctionsBiz{
		...
		public static function trimArrayItems($arr){
			if(gettype($arr) != "array")
				UniteFunctionsBiz::throwError("trimArrayItems error: The type must be array");
		 ...
		 * get url contents
		 */
		public static function getUrlUploadss($url,$arrPost=array(),$method = "post",$debug=false){
			$ch = curl_init();
			$timeout = 0;
			...
			if($response == false) 
				throw new Exception("getUrlUploadss Request failed");
		 ...
		 * get link html
		 */
		public static function getHtmlLink($link,$text,$id="",$class="",$isNew = false){
			...
			$openIn = "";
			if($isNew == true)
				$openIn = "target=\"_blank\" ";
			
			//dmp($text);dmp($isNew);
			
			$html = "<a href=\"$link\"".$id.$class." $openIn>$text</a>";
			return($html);
		}
		...
		/**
		 * 
		 * get new size to fit the biggest size with given ratio
		 */
		public static function getImageSizeByRatio($w, $h, $rX, $rY){
			
			if($rX > $rY){ //16:9, 4:3
				if($w / $rX * $rY > $h){ //image height not high enough
					$w = $h / $rY * $rX;
				}else{
					$h = $w / $rX * $rY;
				}
			}elseif($rX < $rY){ //2:3, 9:16
				if($h / $rY * $rX > $w){ //image width not high enough
					$h = $w / $rX * $rY;
				}else{
					$w = $h / $rY * $rX;
				}
        ...
        /**
		 * 
		 * adds height auto to all images that need to have it.
		 */
		public static function add_height_auto($html) {
            //strip all but image tags
			$temp = trim( strip_tags($html, '<img>') );
			$matches = array();
			preg_match('#(<img.*?>)#', $temp, $matches);
			
			if(!empty($matches)){
				foreach($matches as $images){
                    $attributes = array();
					preg_match_all('/([a-z+\-]*?)="(.*?)"/', $images, $attributes);
					...
					foreach ($attributes[1] as $key => $value) {
                        if(strtolower($value) == 'src'){
                            if($attributes[2][$key] == '[showbiz_image]'){
                                $src_found = true;
                            }else{
                                continue;   
                            }
                        ...
                        if(strtolower($value) == 'style'){
                            $attributes[2][$key] .= " height: auto !important;";
                            $style_found = true;
                        }
                        $attr[$value] = $attributes[2][$key];
                    }
                    if($src_found == true){
                       $new_image = '<img ';
                        ...
                        if($style_found == false){
                            $new_image .= 'style="height: auto;" ';   
                        }
                        $new_image .= '>';
                       
                        $html = str_replace($images, $new_image, $html); 
                    }
                }
            }
            return $html;
		...
		/**
		 * 
		 * check for image tags in text and add lazy loading in it
		 */
		public static function add_lazy_loading($html, $loader_url, $loader_height) {
			//strip all but image tags
			$temp = trim( strip_tags($html, '<img>') );
			$matches = array();
			preg_match('#(<img.*?>)#', $temp, $matches);
			...
			if(!empty($matches)){
				foreach($matches as $images){
					...
					foreach ($attributes[1] as $key => $value) {
						if(strtolower($value) == 'src'){
                            $src_found = true;
				            $attr['data-lazyloadsrc'] = $attributes[2][$key];
                        }else{
                            $attr[$value] = $attributes[2][$key];
                        }
					}
					if($src_found !== false){
						$attr['src'] = $loader_url;
						$attr['data-lazyloadheight'] = $loader_height;
					}
					
					$lazy_image = '<img ';
					foreach($attr as $name => $value){
						$lazy_image .= $name.'="'.$value.'" ';
					}
					$lazy_image .= '>';
					$html = str_replace($images, $lazy_image, $html);
				}
			}
		...
		/**
		 * 
		 * add image tags height and width
		 */
		public static function add_height_width($html, $source_type = 'full', $image_ratio = 'none') {
            global $wpdb;
			//strip all but image tags
			$temp = trim( strip_tags($html, '<img>') );
			$matches = array();
			...
			if(!empty($matches)){
				foreach($matches as $images){
                    ...
                    $src_found = false;
					foreach ($attributes[1] as $key => $value) {
						if(strtolower($value) == 'height' || strtolower($value) == 'width') continue; // if width or height set, do not mess with the image
						if(strtolower($value) == 'src') $src_found = $attributes[2][$key]; // if width or height set, do not mess with the image
                        $attr[$value] = $attributes[2][$key];
					}
                    
                    $img = array();
                    if($src_found !== false && trim($src_found) !== ''){
                        if($source_type == 'custom' || $image_ratio != 'none'){ //take image dimensions from url
                            preg_match_all('/\-[0-9]*x[0-9]*\./', $src_found, $img_matches);
                            
                            if(!empty($img_matches) && isset($img_matches[0][0]) && !empty($img_matches[0][0])){
                                $dimensions_raw = $img_matches[0][0];
                                $dimensions_raw = str_replace(array('-', '.'), '', $dimensions_raw);
                                $dimensions = explode('x', $dimensions_raw);
                                $img = array('', $dimensions[0], $dimensions[1]);
                            }
                        }else{
                            $query = "SELECT ID FROM {$wpdb->posts} WHERE guid='$src_found'";
                            
                            $id = $wpdb->get_var($query);
                           
                            if($id == NULL){
                                $orig_src = preg_replace('/\-[0-9]*x[0-9]*\./', '.', $src_found);
                                $orig_src = preg_replace('/\-[0-9]*x[0-9]*\./', '.', $orig_src);
                                $query = "SELECT ID FROM {$wpdb->posts} WHERE guid='$orig_src'";
                            
                            
                                $id = $wpdb->get_var($query);
                            }
                            $img = wp_get_attachment_image_src($id, $source_type);
                        }
                    }
                    ...
                    if(!empty($img) && isset($img[1]) && isset($img[2])){
                        $new_image .=  'width="'.$img[1].'" ';
                        $new_image .= 'height="'.$img[2].'" ';
                    }
					$new_image .= '>';
					$html = str_replace($images, $new_image, $html);
				}
