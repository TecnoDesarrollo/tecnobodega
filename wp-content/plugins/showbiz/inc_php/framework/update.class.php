...
class UniteUpdateClassBiz {

	private $plugin_url			= 'http://codecanyon.net/item/showbiz-pro-responsive-teaser-wordpress-plugin/4720988';
	private $remote_url			= 'http://updates.themepunch.com/check_for_updates.php';
	private $remote_url_info	= 'http://updates.themepunch.com/showbiz/showbiz.php';
	private $plugin_slug		= 'showbiz';
	private $plugin_path		= 'showbiz/showbiz.php';
	private $version;
	private $plugins;
	private $option;
	private $data;
	...
	protected function _check_updates() {
		...
		$last_check = get_option('showbiz-update-check');
		if($last_check == false){
			$last_check = time();
			update_option('showbiz-update-check', $last_check);
		}
		...
		if(time() - $last_check > 14400){
			...
			if(isset($data->basic)) {
				update_option('showbiz-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;
				
				update_option('showbiz-latest-version', $data->full->version);
			}
	...
	public function _retrieve_version_info() {
		...
		$last_check = get_option('showbiz-update-check-short');
		
		if($last_check == false){
			$last_check = time();
			update_option('showbiz-update-check-short', $last_check);
		}
		...
		if(time() - $last_check > 14400){
			...
			update_option('showbiz-latest-version', $version_info->version);
			update_option('showbiz-update-check-short', time());
		}
