...
var UniteAdminBiz = new function(){
	...
	********************************/
	...
	var initUpdateRoutine = function(){
		...
		jQuery('#sb-validation-activate').click(function(){
			
			UniteAdminBiz.setAjaxLoaderID("rs_purchase_validation");
			UniteAdminBiz.setAjaxHideButtonID("sb-validation-activate");
			
			var data = {
				code: jQuery('input[name="sb-validation-token"]').val(),
				username: jQuery('input[name="sb-validation-username"]').val(),
				api_key: jQuery('input[name="sb-validation-api-key"]').val()
			}
			
			UniteAdminBiz.ajaxRequest("activate_purchase_code",data);
		});
		
		jQuery('#sb-validation-deactivate').click(function(){
			
			UniteAdminBiz.setAjaxLoaderID("rs_purchase_validation");
			UniteAdminBiz.setAjaxHideButtonID("sb-validation-deactivate");
			
			UniteAdminBiz.ajaxRequest("deactivate_purchase_code",'');
