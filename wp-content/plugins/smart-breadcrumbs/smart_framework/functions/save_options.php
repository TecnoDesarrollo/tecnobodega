...
//----------------------------------------------------------------------------//	
// Save text fields
function smartBreadcrumbs_save_text($option) {
    ...
    } elseif (basename($_SERVER['PHP_SELF']) == "categories.php" || trim($__tmp[0]) == "smartBreadcrumbs") {
...
//END smartBreadcrumbs_save_text()

//----------------------------------------------------------------------------//	
// Save default fields
function smartBreadcrumbs_save_default($option) {
    $__tmp = explode("_", $_REQUEST['page']);
...
//END smartBreadcrumbs_save_default()
//----------------------------------------------------------------------------//
// Save checkbox
function smartBreadcrumbs_save_checkbox($option) {
    $__tmp = explode("_", $_REQUEST['page']);
...
//END smartBreadcrumbs_save_checkbox()
//----------------------------------------------------------------------------//
// Save checkbox
function smartBreadcrumbs_save_boxes($option) {
    $__tmp = explode("_", $_REQUEST['page']);
    ...
    for ($k = 1; $k <= $option['count']; $k++) {
        ...
        if (isset($_REQUEST[$id_box])) {
            update_option($id_box, htmlentities($_REQUEST[$id_box]));
        } else {
            delete_option(htmlentities($id_box));
        }
        if (isset($_REQUEST[$id_page]) && $_REQUEST[$id_page] != '') {
            update_option($id_page, htmlentities($_REQUEST[$id_page]));
        } else {
            delete_option(htmlentities($id_page));
        }
        if (isset($_REQUEST[$id_post]) && $_REQUEST[$id_post] != '') {
            update_option($id_post, htmlentities($_REQUEST[$id_post]));
        } else {
            delete_option(htmlentities($id_post));
        }
    }
}

//END smartBreadcrumbs_save_boxes()
//----------------------------------------------------------------------------//   
// Save multicheckbox
function smartBreadcrumbs_save_multicheck($option) {
    $__tmp = explode("_", $_REQUEST['page']);
    $id = $option['id'];
    ...
    if (basename($_SERVER['PHP_SELF']) == "admin-ajax.php" && basename($_REQUEST['_wp_http_referer']) == "categories.php") {
        ...
        foreach ($option['options'] as $key => $value) {
            ...
            if (isset($_POST[$up_opt])) {
                update_option($up_opts, $_POST[$up_opt]);
            } else {
                update_option($up_opts, 'false');
            }
        }
    } elseif (basename($_SERVER['PHP_SELF']) == "categories.php" || trim($__tmp[0]) == "smartBreadcrumbs") {
        ...
        foreach ($option['options'] as $key => $value) {
            ...
            $up_opt = $id . '_' . $key;
            if (isset($_POST[$up_opt])) {
                update_option($up_opt, $_POST[$up_opt]);
            } else {
...
//END smartBreadcrumbs_save_multicheck()
//----------------------------------------------------------------------------// 

function smartBreadcrumbs_save_multi($option) {
    $__tmp = explode("_", $_REQUEST['page']);
    ...
    if (basename($_SERVER['PHP_SELF']) == "admin-ajax.php" && basename($_REQUEST['_wp_http_referer']) == "categories.php") {
        ...
        while (smartBreadcrumbs_option_exist($ids . "_" . $i)) {
            if (!isset($_REQUEST[$id . "_" . $i]))
                delete_option($ids . "_" . $i);
            $i++;
        }
        ...
        //delete old data that not ben send this time
        while (smartBreadcrumbs_option_exist($id . "_" . $i)) {
            if (!isset($_REQUEST[$id . "_" . $i]))
                delete_option($id . "_" . $i);
            $i++;
        }
        ...
        //delete old data that not ben send this time
        while (smartBreadcrumbs_meta_exist($id . "_" . $i, $post_id)) { //update_post_meta($post_id , $id . '_aaaaaa'.$i, $id."_".$i );
            if (!isset($_REQUEST[$id . "_" . $i]))
                delete_post_meta($post_id, $id . "_" . $i, get_post_meta($post_id, $id . "_" . $i, true));
            $i++;
        }
...
//END smartBreadcrumbs_save_multi()
//----------------------------------------------------------------------------// 

function smartBreadcrumbs_save_slider($option) {
    $__tmp = explode("_", $_REQUEST['page']);
    ...
    if ($slider_type == 'upload') {
        ...
        if ($uploaded_imgs) {
            ...
            if (basename($_SERVER['PHP_SELF']) == 'post.php' || basename($_SERVER['PHP_SELF']) == 'page.php') {
                ...
                while (smartBreadcrumbs_meta_exist($id . '_sliderdata_' . $i, $_POST['post_ID'])) {
                ...
                while (smartBreadcrumbs_option_exist($id . '_sliderdata_' . $i)) {
                    if (!isset($_REQUEST[$id . '_sliderdata_' . $i]))
...
//END smartBreadcrumbs_save_slider()