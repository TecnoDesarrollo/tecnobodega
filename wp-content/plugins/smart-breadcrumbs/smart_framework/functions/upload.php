...
if (!function_exists('smartBreadcrumbs_upload')) {

    function smartBreadcrumbs_upload($option) {
        global $post, $smartBreadcrumbs;
		$uploader = ''; // avoid notice
        $upload = $option ['std'];
        $id = $option ['id'];
        $type = $option ['type'];
		$val = '';
        ...
        } elseif (smartBreadcrumbs_option_exist($id)) {
        ...
        if (!empty($upload) and $type == 'upload') {
            $uploader .= '<img id="image_' . $id . '" src="' . $smartBreadcrumbs->cfg('PLUGIN_URI') . '/thumb.php?src=' . $upload . '&w=120&h=60&zc=1" class="smartBreadcrumbsPreviewThumb" alt="" target="_blank" />';
        }
        $uploader .= '</a>';
        $uploader .= '<div class="clear"></div>' . "\n";
...
// END smartBreadcrumbs_upload



    add_action('wp_ajax_smart_ajax_post_action', 'smart_ajax_upload_callback');
    if (!function_exists('smart_ajax_upload_callback')) {
        ...
        function smart_ajax_upload_callback() {
            ...
            if ($_POST ['page'] == 'post.php' || $_POST ['page'] == 'post-new.php') {
                $options = get_option("{$smartBreadcrumbs->prefix}_post_options");
            } elseif ($_POST ['page'] == 'page.php' || $_POST ['page'] == 'page-new.php') {
                $options = get_option("{$smartBreadcrumbs->prefix}_page_options");
            } elseif ($_POST ['page'] == 'admin.php') {
                $options = get_option("{$smartBreadcrumbs->prefix}_admin_options");
            } else
                $options = array();
