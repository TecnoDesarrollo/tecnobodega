...
function vw_setup_sidebar_generator() {
	...
	add_shortcode( 'sidebar', 'vw_sidebar_shortcode' );
}
...
class SidebarGenerator {
		...
		if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					'name'          => $sidebar,
					'description'   => __( 'These are widgets.', 'redthemesv-backend' ),
					'before_widget' => '<div id="%1$s" class="widget vw-sidebar-custom-'.$sidebar_class.' %2$s">',
					'after_widget'  => '</div>',
	...
	static function add_sidebar(){
		...
		if ( ! is_array( $sidebars ) ) $sidebars = array(); // Re-initial to avoid the problem
	...
	static function admin_page(){
		...
		<div class="wrap">
			...
			<p>
				...
				You can create how many sidebars you want and then go and <a href="post-new.php?post_type=page">Create</a> or <a href="edit.php?post_type=page">Edit</a> a page and asign this custom sidebar. You can assign it even to a post or anything else just go to a page, post or Opciones del Tema and find the otion to select a sidebar.<strong>The shortcode is provided optional in case if you need it. Is not necesary to use it.</strong><br />
			...
			<table class="widefat page" id="sbg_table" style="width:100%;">
				...
				<tr class="<?php echo $alt?>">
					...
					<td style="padding-top: 10px; padding-bottom: 10px;"><code style="padding: 4px 7px; border: 1px solid #bbb;">[sidebar name="<?php echo $sidebar; ?>"]</code></td>
...
function vw_sidebar_shortcode( $atts ) {
	...
	return smk_custom_dynamic_sidebar($name);
}
