...
class Vw_widget_custom_banner extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			'redthemesv Custom Banner', // Name
			array( 'description' => __( 'Display banner', 'redthemesv-backend' ), ) // Args
		);
	}
	...
	function widget( $args, $instance ) {
		...
		if ( function_exists( 'icl_t' ) ) {
			...
			$instance['buttontext'] = icl_t( 'RedSV Widget', $this->id.'_buttontext', $instance['buttontext'] );
		}

		$supertitle_html = '';
		if ( ! empty( $instance['supertitle'] ) ) {
			$supertitle_html = sprintf( __( '<h6 class="banner-supertitle">%s</h6>', 'redthemesv' ), $instance['supertitle'] );
		}

		$title_html = '';
		if ( ! empty( $instance['title'] ) ) {
			$title = apply_filters( 'widget_title', $instance['title'], $instance, $this->id_base);
			$title_html = sprintf( __( '<h3 class="banner-title">%s</h3>', 'redthemesv' ), $title );
		}

		$subtitle_html = '';
		if ( ! empty( $instance['subtitle'] ) ) {
			$subtitle_html = sprintf( __( '<div class="banner-subtitle">%s</div>', 'redthemesv' ), $instance['subtitle'] );
		}

		$button_html = '';
		if ( ! empty( $instance['buttontext'] ) ) {
			$button_html = sprintf( __( '<a href="%s" class="banner-button read-more label label-large primary-bg">%s</a>', 'redthemesv' ), $instance['linkurl'], $instance['buttontext'] );
		}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( function_exists( 'icl_register_string' ) ) {
			...
			icl_register_string( 'RedSV Widget', $this->id.'_buttontext', $instance['buttontext'] );
		}
...
?>
		...
		<!-- subtitle -->
		<p>
			<label for="<?php echo $this->get_field_id('subtitle'); ?>"><?php _e('Subtitle:','redthemesv-backend'); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id('subtitle'); ?>" name="<?php echo $this->get_field_name('subtitle'); ?>" type="text" value="<?php echo esc_attr($subtitle); ?>" />
		</p>

		<!-- image url -->
		<p>
			<label for="<?php echo $this->get_field_id('imgurl'); ?>"><?php _e('Image Url:','redthemesv-backend'); ?></label>
		...
		<!-- link url -->
		<p>
			<label for="<?php echo $this->get_field_id('linkurl'); ?>"><?php _e('Link Url:','redthemesv-backend'); ?></label>
		...
		<!-- button text -->
		<p>
			<label for="<?php echo $this->get_field_id('buttontext'); ?>"><?php _e('Button Text:','redthemesv-backend'); ?></label>
